...
def main():
    ...
    write_json(output / 'run.json', dict(config=config, command=sys.argv, protocol='outer5_inner20'))
    ...
    for task in TASKS:
        ...
        try:
            ...
            for fold in manifest['folds']:
                ...
                set_global_seed(config['seed'] + fold_id)
                ...
                def loader(indices, training=False):
                    return DataLoader(Subset(dataset, indices),
                        batch_size=config['finetune_batch'] if training else config['eval_batch'],
                ...
                encoder = build_dual_glt_model(config['fusion_mode']).to(device)
                load_deployment(encoder, package, config['downstream_step'])
                model = EvaluationAdapter(encoder)
                optimizer = optimizer_for(encoder, config)
                scheduler = _cosine_scheduler(optimizer, config['epochs'] * len(train_loader),
                                               config['finetune_warmup'] * len(train_loader))
                ...
                save_checkpoint(folder / 'best.pt', dict(state_dict=best, fusion_mode=config['fusion_mode'],
                    architecture=encoder.architecture_name, task=task, fold=fold_id, protocol='outer5_inner20',
                    split=fold, config=config, best_validation_r2=best_r2, best_epoch=best_epoch,
    ...
    write_json(output / 'summary.json', dict(protocol='outer5_inner20', tasks=task_summary,
