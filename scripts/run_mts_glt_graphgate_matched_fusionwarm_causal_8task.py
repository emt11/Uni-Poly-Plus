#!/usr/bin/env python3
"""Run the remaining matched-5k Full/Off fold-0/1/2 expansion units."""
...
LOGS = ROOT / (
    "logs/mts_glt_graphgate_v1/trimer_validation_v1/matched_5k/"
    "fusionwarm_causal_v1/eight_task_fold012"
)
SIDECAR = ROOT / "data/processed/mips_trimer_scage/periodic_line_glt_central_v1/downstream_union"
FORBIDDEN = ROOT / "results/mts_glt_graphgate_v1/pretrain/mts_glt_graphgate_probe_005k.pth"


def _checkpoint(path: str) -> Path:
...
def _scheduler(*, arm, mode, checkpoint, tasks, folds, gpu_ids, config):
    ...
    command = [
        sys.executable,
        "scripts/run_mts_finetune_scheduler.py",
        "--python",
        sys.executable,
        "--gpu-ids",
        gpu_ids,
        ...
        *[str(value) for value in folds],
        ...
        "--train-args",
        "--experiment_id",
        f"mts_glt_graphgate_matched_fusionwarm_8task_fold012_v1_{arm}",
        "--config_schema",
        ...
        "--mts_glt_layers",
        "6",
        "--mts_glt_attention_variant",
        "mips",
        "--mts_glt_mode",
        mode,
        "--mts_glt_geometry_mode",
        arm,
        ...
        str(config["graph_lr"]),
        "--fusion_lr",
        str(config["fusion_lr"]),
        "--head_lr",
        str(config["head_lr"]),
        "--regression_loss",
        str(config["regression_loss"]),
        "--huber_beta",
    ...
    if mode == "o8_glt_graph":
        command.extend(
            [
                ...
                "fusion_warm",
                "--mts_glt_fusion_warm_epochs",
                str(config["fusion_warm_epochs"]),
                "--mts_glt_initial_alpha",
                str(config["initial_alpha"]),
                "--mts_glt_fusion_warm_dir",
                str(result_root),
                "--warmup_epochs",
                "0",
            ]
...
def main(argv=None):
    ...
    expected_tasks = ["eat", "eea", "egb", "egc", "ei", "eps", "nc", "xc"]
    if config["tasks"] != expected_tasks or config["folds"] != [0, 1, 2]:
        raise RuntimeError("canonical task/fold roster changed")
    ...
    calls = (
        ("full", "o8_glt_graph", full, tuple(config["new_tasks"])),
        ("off", "o8_glt_graph", off, tuple(config["new_tasks"])),
        ("full", "o8_only", full, tuple(config["new_o8_tasks"])),
        ("off", "o8_only", off, tuple(config["new_o8_tasks"])),
    )
    for arm, mode, checkpoint, tasks in calls:
        status = _scheduler(
            arm=arm,
            mode=mode,
    ...
    return subprocess.call(
        [sys.executable, "scripts/report_mts_glt_graphgate_matched_fusionwarm_causal_8task.py"],
        cwd=ROOT,
    )
