#!/usr/bin/env python3
"""Run matched Full/Off O8/FusionWarm folds 3/4 and produce a five-fold report."""
...
def _scheduler(*, arm, mode, checkpoint, tasks, folds, gpu_ids, config):
    ...
    command = [
        ...
        *[str(value) for value in folds],
        ...
        "--train-args",
        "--experiment_id",
        f"mts_glt_graphgate_matched_fusionwarm_8task_5fold_v1_{arm}",
        "--config_schema",
...
def main(argv=None):
    ...
    if config["tasks"] != expected_tasks or config["new_folds"] != [3, 4]:
        raise RuntimeError("canonical task/new-fold roster changed")
    ...
    calls = (
        ("full", "o8_glt_graph", full),
        ("off", "o8_glt_graph", off),
        ("full", "o8_only", full),
        ("off", "o8_only", off),
    )
    for arm, mode, checkpoint in calls:
        status = _scheduler(
            arm=arm,
            mode=mode,
            checkpoint=checkpoint,
            tasks=tuple(config["tasks"]),
            folds=tuple(config["new_folds"]),
    ...
    return subprocess.call(
        [sys.executable, "scripts/report_mts_glt_graphgate_matched_fusionwarm_causal_5fold.py"],
        cwd=ROOT,
    )
