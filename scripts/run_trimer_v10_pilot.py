#!/usr/bin/env python3
"""Run the bounded, CPU-only Trimer-v10 300-record real-data pilot.

v10 decouples source/base identity (which may contain explicit isotope H)
from the Z>1 heavy subset; this rerun uses the identical deterministic
selection and generation parameters as the blocked v9 pilot.
"""
...
def analyze(samples, summaries, candidates, comparisons):
    ...
    result = {
        ...
        "stereo": stereo,
        "isotope": {
            "samples_with_source_explicit_h": sum(
                bool(row.get("source_explicit_h_count")) for row in summaries
            ),
            "samples_with_source_isotope_h": sum(
                bool(row.get("source_isotope_h_count")) for row in summaries
            ),
            "total_source_h": sum(row.get("source_explicit_h_count") or 0 for row in summaries),
            "total_source_deuterium": sum(row.get("source_deuterium_count") or 0 for row in summaries),
            "total_source_tritium": sum(row.get("source_tritium_count") or 0 for row in summaries),
            "isotope_identity_contract_errors": sum(
                row.get("failure_category") == "TRIMER_CONTRACT_ERROR"
                and "identity" in str(row.get("error", "")) for row in summaries
            ),
            "isotope_samples_geometry_success": sum(
                bool(row.get("source_isotope_h_count")) and row["geometry_valid"]
                for row in summaries
            ),
            "isotope_samples_geometry_failure": sum(
                bool(row.get("source_isotope_h_count")) and not row["geometry_valid"]
                for row in summaries
            ),
        },
        "mmff": {
...
def report_markdown(report):
    b, e, r, s, m, h, t, iso = (
        ...
        report["stereo"], report["mmff"], report["explicit_h"],
        report["timing_seconds"], report["isotope"],
    )
    decision = (
        "3. 当前存在必须先修复的问题"
    ...
    return f"""# Trimer v10 有限真实记录 pilot（isotope 契约修复后重跑）
...
## F2. Isotope H（v10 身份契约）

- 含 source explicit H 样本：{iso['samples_with_source_explicit_h']}；含 isotope H：{iso['samples_with_source_isotope_h']}。
- total source H / D / T：{iso['total_source_h']} / {iso['total_source_deuterium']} / {iso['total_source_tritium']}。
- isotope identity contract errors：{iso['isotope_identity_contract_errors']}。
- isotope 样本 geometry success / failure：{iso['isotope_samples_geometry_success']} / {iso['isotope_samples_geometry_failure']}。
- 目标是 isotope identity contract error = 0，而非 isotope geometry 100% 成功。
...
def main():
    ...
    try:
        for position, sample in enumerate(samples, 1):
            ...
            source_count = int(diagnostics.get("trimer_source_atoms", 0))
            src_atomic = record.trimer_atomic_number[record.is_source_atom]
            src_isotope = record.trimer_isotope[record.is_source_atom]
            source_h_count = int((src_atomic == 1).sum())
            isotope_h_count = int(((src_atomic == 1) & (src_isotope > 0)).sum())
            deuterium_count = int(((src_atomic == 1) & (src_isotope == 2)).sum())
            tritium_count = int(((src_atomic == 1) & (src_isotope == 3)).sum())
            round_map = {int(row["round_id"]): row for row in diagnostics.get("rounds", [])}
            summary = {
                "sample_id": sample["sample_id"], "canonical_smiles": canonical,
                "source_atom_count": source_count or None,
                "heavy_atom_count": heavy_count,
                "source_explicit_h_count": source_h_count or None,
                "source_isotope_h_count": isotope_h_count or None,
                "source_deuterium_count": deuterium_count or None,
                "source_tritium_count": tritium_count or None,
