#!/usr/bin/env python3
"""Run SL and X2L line-conditioning screening arms and report."""
...
CONFIG = ROOT / 'configs/mts/glt_v2_x2l_line_conditioning_screen_v1.json'


def _command(config, arm, gpu_ids, smoke=False):
    mode = {
        'sl': 'o8_glt_atom_line_self',
    ...
    return [
        ...
        '--results-dir', str(result_root), '--logs-dir', str(logs / suffix),
        ...
        '--mts_glt_interaction_diagnostics_dir', str(output / 'line_units' / suffix),
...
def main(argv=None):
    ...
    if not args.report_only:
        subprocess.run([
            sys.executable, 'scripts/check_mts_glt_v2_line_conditioning_sanity.py',
    ...
    return subprocess.call([
        sys.executable, 'scripts/report_mts_glt_v2_x2l_line_conditioning_screen.py'
    ], cwd=ROOT)
