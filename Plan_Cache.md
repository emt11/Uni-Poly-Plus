...
| 状态 | 需返修；r1 已有实现与报告，但 Codex 审查未通过；r2 返修尚未执行 |
| 当前授权 | 本轮用户明确要求将下一步写入本文件，Codex 仅修改文档。r1 执行授权来源保留 Plan.md 中 ZCode 的记录；原已授权范围内的局部修复／必要验证不重复申请，不由 r2 扩大预算或授权生产切换 |
| 角色 | Codex 规划与对 ZCode 交付的审查；ZCode 或用户指定执行者返修。本轮文档修改与自检均为 Codex，不代表返修已执行／独立验收通过 |
...
本轮文档交付不等于上述步骤完成。r2 周期继续“需返修”，不将 r1 自检归档为最终验收通过。
