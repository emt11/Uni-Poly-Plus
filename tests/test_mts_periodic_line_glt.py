"""Focused unit coverage for the retained MTS-GLT-v2 baseline path."""
...
from src.dataset.periodic_line_glt import build_periodic_line_sample, canonical_line_token
...
def test_rigid_transform_preserves_line_geometry_observations():
    topology = _sample()
...
def test_v2_forward_ignores_bond_type_token_input():
    samples = [_sample("*CCO*"), _sample("*COC*")]
    records = [
        build_periodic_line_sample(sample_key_from_smiles(smiles), sample, sample)
        for smiles, sample in (("*CCO*", samples[0]), ("*COC*", samples[1]))
    ]
    batch = mips_trimer_collate([_attach(samples[0], records[0]), _attach(samples[1], records[1])])
    encoder = LocalPeriodicGraphLineTransformerV2(layers=1, hidden_size=64, heads=8).eval()
    with torch.no_grad():
...
def test_encode_then_mean_is_not_mean_then_encode():
    ...
    model = LocalPeriodicGraphLineTransformerV2(layers=1, hidden_size=64, heads=8)
    dual = torch.nonzero(batch.glt_token_observation_count == 2).flatten()
    assert dual.numel() == 1
    index = int(dual[0])
    values = batch.glt_token_observation_distances[index:index + 1]
    encoded = model.distance_basis(
        values,
        torch.tensor([2]),
        z_a=batch.glt_token_endpoint_z_a[index:index + 1],
        z_b=batch.glt_token_endpoint_z_b[index:index + 1],
    )
    wrong_values = values[:, :2].mean(dim=1, keepdim=True).expand(-1, 3)
    wrong = model.distance_basis(
        wrong_values,
        torch.tensor([1]),
        z_a=batch.glt_token_endpoint_z_a[index:index + 1],
        z_b=batch.glt_token_endpoint_z_b[index:index + 1],
    )
    assert not torch.allclose(encoded[0], wrong[0])
...
def test_ddp_infonce_gathers_fixed_shapes_before_filter(tmp_path):
    mp.spawn(_infonce_worker, args=(str(tmp_path / "infonce_init"),), nprocs=2, join=True)
...
def test_glt_v2_observation_moments_and_identity_self_relations():
    ...
    assert bool((source[-token_count:] == target[-token_count:]).all())
    assert output["atom_geometry_states"].shape[0] == int(batch.canonical_graph_index.numel())
    assert bool(output["atom_geometry_valid"].any())


def test_balanced_masking_uses_exact_count_and_whole_tokens():
    topology = _sample()
    record = build_periodic_line_sample(sample_key_from_smiles("*CCO*"), topology, topology)
    batch = mips_trimer_collate([_attach(topology, record)])
    frequencies = torch.ones(NUM_LINE_LABELS, dtype=torch.long)
    inputs = make_masked_line_inputs_v2(
        batch, 0.40, frequencies, torch.Generator().manual_seed(5)
    )
...
def test_invalid_geometry_fusion_is_exact_o8():
    samples = [_sample("*CCO*"), _sample("*COC*")]
    records = [
        build_periodic_line_sample(sample_key_from_smiles(smiles), sample, sample)
        for smiles, sample in (("*CCO*", samples[0]), ("*COC*", samples[1]))
    ...
    model = MTSGraphLineModelV2().eval()
    with torch.no_grad():
...
def test_finetune_builds_baseline_and_strictly_maps_probe_state(monkeypatch):
    ...
    checkpoint_state = {"model." + key: value.clone() for key, value in encoder.state_dict().items()}
