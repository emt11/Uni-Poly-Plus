...
- 四个预训练部署包 SHA 均匹配本计划表；CPU strict-load：GLT_REF、M_CAT、M_GATE、M_XATTN 全部 PASS；O8_ONLY strict-load 后复制 83/83 个张量。O8_ONLY 与三个 MCL arm 的 512-wide head 初值 SHA 一致，为 `8fde47d5b69ad5c5215abfa902726005c88dcb260b858f0339eda30edc77ae2d`。统计 SHA 为 `9dc8160f1de5152f6c04a963c40569bac8facd21e68a700f40186363798cf8b1`；config、cache 与 static manifest 存在。development 输出根不存在；在 `logs/mcl_ph_20260921/` 下预期的 30 组 unit `.log`/`.exit` 文件共 60 个，当前全部不存在。
