...
**当前 r10R3（2026-09-23，执行中）**：r10R3 修复、P0 与 cache 前置验收见 §13.17；CAT 已完成 5,000 updates 并通过退出码、runtime/cleanup、千步文件、统计/共同初值身份及 CPU strict-load 验收。本轮用户授权范围仅为 GATE、XATTN 各一次、最多 5,000 updates，按 GATE 完整验收后再启动 XATTN。当前执行和逐臂证据见 [Plan.md](Plan.md) 与 §13.18。development、P3、OOF、outer-test 均不在本轮执行范围。以下 r10R1 状态段落为保留的历史记录。
