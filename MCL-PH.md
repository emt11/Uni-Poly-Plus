...
**当前 r10R3（2026-09-23，预训练完成；development 待执行）**：r10R3 修复、P0 与 cache 前置验收见 §13.17；GLT_REF、CAT、GATE、XATTN 四条正式 5k 轨迹均已完成并通过对应包的身份和严格加载核对，旧失败预算继续保留。GATE/XATTN 执行记录见 §13.18，XATTN 收尾审查见 §13.19。用户已授权 30-unit development 阶段，但尚未启动任何 unit；§13.20–13.21 的旧阻断记录保留。DEV2 新增受冻结 cohort 绑定的行偏移索引，使训练读取路径只请求当前折 train/validation 记录；8×5 微调器能力已实现，**8×5 运行预算仍未授权**，详见 §13.22 和 [Plan.md](Plan.md)。P3、OOF、refit、outer-test 均未启动或获本计划授权。以下 r10R1 状态段落为保留的历史记录。
