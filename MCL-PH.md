...
**当前 r10R3（2026-09-23，执行中）**：用户本轮明确要求实施「先修复再训练」方案，授权新增 CAT/GATE/XATTN 各最多一次 5000 updates（合计最多 15,000；历史 10,082 不清零）、其后最多 30 个 development units / 900 epochs。GLT_REF 复用 `glt_ref_r10r1` 的 5k 包；旧 CAT step4928 现场不恢复、不覆盖。已修非键 clean target 的重原子索引、千步 checkpoint、备用分母导入、O8_ONLY 重复 LayerNorm；定向局部检查通过，新 P0 4096 样本统计 PASS，完整 cache 校验通过，正式 CAT 新轨迹已启动。具体证据与停止条件见 [Plan.md](Plan.md) 及 §13.17。P3、OOF 与 outer-test 不在本轮授权内。以下 r10R1 状态段落为保留的历史记录。
