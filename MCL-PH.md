...
> 2026-09-25 当前修订为 [Plan.md](Plan.md) 的 `r13-PAPER5`。用户取消 r12 项目自建五折 outer-test 运行并要求改用 Periodic-TDL 论文官方五折；r12 首批四个 unit 失败后 launcher 已退出，当前无微调进程。官方发布样本与冻结 cohort 逐行一致的任务只有 Eea/Egb/Ei/EPS/Nc，五任务官方 outer/inner 索引已生成并局部验证；EAT、Egc、Xc 不纳入同折可比结果。用户现已明确授权以 `paper5_outer` 全新重跑五臂 × 五任务 × 五折，共 125 units / 最多 8,750 epochs；旧活动微调路径将移除，正式运行已在 `Uni-Poly:96:mcl_ph_paper5_4gpu` 启动，125 units 尚未完成；训练完成与 outer-test R² 以本轮实际日志和产物为准。下文 r10–r12 状态为历史记录，不授权自动重启。
...
计划 ID：`MCL-PH-20260921-01` ｜ 当前修订：r10R3 ｜ 日期：2026-09-23 UTC

## 0. 状态、角色与授权

**当前 r10R3（2026-09-23，预训练完成；五臂 8×5 微调执行中）**：r10R3 修复、P0 与 cache 前置验收见 §13.17；GLT_REF、CAT、GATE、XATTN 四条正式 5k 轨迹均已完成并通过对应包的身份和严格加载核对，旧失败预算继续保留。GATE/XATTN 执行记录见 §13.18，XATTN 收尾审查见 §13.19。§13.20–13.22 的旧阻断与索引修复记录保留；用户现已明确授权五臂全部的 8 任务×5 折微调与内部验证，最多 200 units / 6,000 epochs，**不读取 outer-test**，执行细则见 [Plan.md](Plan.md)。原 30-unit development 未启动，不另加额度；P3、OOF、refit、outer-test 仍未授权。以下 r10R1 状态段落为保留的历史记录。
...
### 13.17 r10R3 修复及 P2 执行（Codex 执行；2026-09-23 UTC；基准 `dev@29e359e`）

用户本轮明确要求实施 r10R3，故本节覆盖了此前 r10R1 的停止边界；先修复、核对统计与身份，再按新预算执行。历史失败和已消耗预算不回写。计划与逐阶段实时执行记录见 [Plan.md](Plan.md)。本轮实际执行者为 Codex；自己的测试与核对属于自检，没有第二主体的独立审查。

| 项目 | 本轮固定要求 |
| --- | --- |
| 修复 | 非键 clean target 使用重原子视图索引；训练和固定 P0 统计同源；千步 resume/deploy；备用分母 `global_sum`；O8_ONLY 共享 head 前仅一层 LayerNorm |
| P0 | 固定 P_train 4096 样本，写入新目录 `results/mcl_ph_20260921/p0_r10r3/`；旧统计与审计不覆盖；完整拓扑 cache 只读复用 |
| 正式预训练 | GLT_REF 复用 `glt_ref_r10r1` 成功 5k；CAT、GATE、XATTN 分别从原始共同初始化串行启动一次、各最多 5000 updates，输出到 `{cat,gate,xattn}_r10r3/`；不设墙钟 timeout，不从 CAT 4928 续训 |
| 微调 | 五臂 × XC/EPS/EAT × fold0/1，最多 30 units / 900 epochs；仅 development validation 选 epoch；不读 outer-test |
| 停止 | 任一目标、身份、统计、cache、checkpoint、有限性、退出码或预算失败，停止依赖阶段；不中断后自动重启或扩大预算 |
| 结论边界 | P2 聚合只属于 development 筛查；不自动启动 P3、OOF 或 outer-test，不从 smoke 的 R² 排名 |

执行状态（写入时）：修复已落下；初始 23 项定向检查通过，另加失败不伪报 PASS 用例后 5/5 通过。新 P0 统计 exit 0、`PASS`、4096/4096、固定 key SHA 一致、各数组有限；nonbond `(mu,sigma)=(1.3945351,0.1372659)`，旧统计 `(1.3962990,0.1445435)`，新文件 SHA `9dc8160f…cf8b1`。51 个 trajectory shard 的 SHA256 校验及 5,040,000 位置覆盖通过，cohort/static/sample-index 身份与 P0 一致。预检代码提交 `31ff03f` 已推送并核实远端。CAT 在 `Uni-Poly:mcl_ph_r10r3_cat` 以全新目录 `p2/pretrain/cat_r10r3/` 启动，当前只可称**运行中**；GATE、XATTN、development 均未启动。用户于 2026-09-23 01:06 UTC 要求将监控交给其他模型，等待中的 `scripts/run_mcl_ph_r10r3.py` 已有意停止，CAT 训练保持运行。后续阶段不会由该执行器自动启动；接手者须先核对真实状态，再按门槛继续。后续证据在阶段结束后追加，不把目标写成结果。

### 13.18 r10R3 GATE/XATTN 正式预训练继续执行（2026-09-23 UTC）

用户本轮明确要求继续同一计划，仅串行完成 GATE 与 XATTN 两臂正式预训练，各最多 5,000 updates、各一次启动；不启动 development、P3、OOF 或 outer-test，也不重启包含 development 阶段的 `scripts/run_mcl_ph_r10r3.py`。基准为 `dev@73a1980`；修改文档前 `git pull --ff-only origin dev` 返回 Already up to date。当前分支与 `origin/dev` 同步，初始工作区干净；`.zcodeignore` 删除已属于基准提交，不是本轮改动。

**前置现场核对（启动 GATE 前）**：

- CAT 的真实退出码文件 `logs/mcl_ph_20260921/p2_cat_r10r3_pretrain.exit` 内容为 `0`。`runtime.json` 为 PASS / 5,000，cleanup complete、export complete、main returned；`scripts/verify_mcl_ph_arm.py --label cat_r10r3 --arm-dir results/mcl_ph_20260921/p2/pretrain/cat_r10r3 --updates 5000 --strict-cleanup` 返回 PASS。五组 resume/deploy 文件均存在。
- CAT `deploy_05000.pt` 为 step=5000、fusion_mode=cat、training_route=mcl_ph；其包 SHA256 为 `eed6276565f0bc827fc1f56056a25cd8469db184b0e334d04c3dd0f3cfea0e49`。使用 `build_mcl_arm('cat', package, expected_step=5000)` 的 CPU 严格加载通过，encoder 参数 20,336,451。
- 统计文件实际 SHA256 为 `9dc8160f1de5152f6c04a963c40569bac8facd21e68a700f40186363798cf8b1`；shared new init 实际 SHA256 为 `499309392d578daf7a7baf1d102d7a7f5c652e5a9b42ca2904ac9d83d1fab85a`；common init artifact 实际 SHA256 为 `1c9f97cf5547dcd2f44846e83586dfb4ec593dd4a4f56df510be372f26f1951d`。三者均与 CAT runtime identity 一致。
- 当前 trajectory-cache manifest 显示 complete=true、completed_shards=51、total_positions=requested_total_positions=5,040,000；51 个 shard 文件连续覆盖 `[0, 5,039,999]`。seed=42、noise_sigma=0.03、mask_ratio=0.3、global_batch=1008、train sample-index SHA、cohort hash、dual-static hash 均与 CAT runtime identity 一致；既有 `logs/mcl_ph_20260921/trajectory_cache_verify.log` 记录 `FULL CACHE VERIFY OK`。
- CAT/GATE/XATTN 三配置逐字段比较只有 `fusion_mode` 不同；要求的 world=4、84×3、global batch=1008、BF16、lr=2e-4、warmup=2,000、scheduler=20,000、dense 500→Top-2 501、5,000 updates 均一致。GATE、XATTN 新输出目录均不存在。
- `Uni-Poly` 有效；启动前没有活动 MCL-PH 训练进程。四张 RTX 4090 利用率为 0%，显存空闲约 48 GiB/卡。

**本轮计划与实际阶段状态（本段将在每阶段结束后追加实际证据）**：

1. GATE：配置 `configs/mts/mcl_ph_gate.json`；独立 window `Uni-Poly:mcl_ph_r10r3_gate`；输出 `results/mcl_ph_20260921/p2/pretrain/gate_r10r3/`；日志 `logs/mcl_ph_20260921/p2_gate_r10r3_pretrain.log`；退出码 `logs/mcl_ph_20260921/p2_gate_r10r3_pretrain.exit`。执行命令为用户本轮给出的 `python3 -m torch.distributed.run --standalone --nproc_per_node=4 scripts/pretrain_mcl_ph.py` 加 GATE 配置、固定统计/共同初值/cache、`--diagnostics --prep-workers 12 --stop-after-step 5000` 和指定输出路径。**此记录写入时尚未启动**；启动和完成后填写实际起止时间、命令、退出码、五组文件、路由/有限性、包 SHA 与 strict-load/verifier 结果。
...
### 13.19 r10R3 正式预训练收尾核对与下一步（Codex；2026-09-23 UTC）

本次审查以 `dev@d4f31ee` 为基准，只读核对实际产物、退出码、日志、配置与当前代码；未启动或恢复训练，未读取 outer-test。`git status` 干净、`origin/dev` 与本地一致，`p2/development_r10r3/` 尚不存在，无 MCL-PH 训练进程。

- XATTN 的真实退出码 `logs/mcl_ph_20260921/p2_xattn_r10r3_pretrain.exit=0`（写入时间 10:30:17 UTC），runtime 为 `PASS/5000`、world=4、cleanup complete、export_complete=true、main_returned=true。五组 1000–5000 的 resume/deploy 均齐；step 500 dense、501 top2，1000–5000 均 top2。`scripts/verify_mcl_ph_arm.py --strict-cleanup` 返回 0 / PASS。
- XATTN 的完整 stdout 日志中四 rank 各有恰好 5000 个不同 step 前缀，范围 1–5000；多进程输出会使部分完整 JSON 行交错，所以逐行解析得到的条数少于真实打印的 step 前缀数。可解析记录和全文均无 NaN/Inf 标记；runner 本身对非有限 loss 与梯度硬失败。不能把日志行交错当作训练缺步，也不能把无法完整解析的行声称已逐字段验收。
- CAT/GATE/XATTN 的最终部署包均以对应 `build_mcl_arm(..., expected_step=5000)` 严格加载，router 推理模式为 top2；三个包的统计 SHA、共享新初值 SHA 和 common-init SHA 与各自运行身份一致。XATTN 包 SHA256 为 `9f2bea298b7341eaaf02e85a84f95974334e64b5436310127d30d82ae49f206e`。GLT_REF 的 `deploy_05000.pt` strict-load 通过，O8_ONLY 从同包复制 O8 张量 83/83，512 维 head 初值一致。四包完整 SHA 见新 [Plan.md](Plan.md)。
- P2 聚合器的针对性测试 `tests/test_mcl_ph_p2_aggregate.py` 为 15 passed；这只验证聚合逻辑，尚无 development 结果。累计正式预训练预算为 **25,082 updates / 6 次启动**（含历史 154 与 4928 步失败）；development 为 0/30 units、0/900 epochs。

**审查结论：正式预训练产物验收通过，可进入 P2 development 的启动准备。** 本轮只制定 `r10R3-DEV1` 计划，状态待授权；没有启动 30 units 或聚合，也没有预测性能增益结论。GATE/XATTN 周期摘要归档于 `PROJECT_HISTORY.md`，下一步实施顺序、预算、门槛和停止条件见 `Plan.md`。

### 13.20 r10R3-DEV1 development 执行前置检查阻断（Codex；2026-09-23 UTC）

用户于 2026-09-23 UTC 明确授权仅执行 `MCL-PH-20260921-01 / r10R3-DEV1` 的 P2 development 阶段，预算为五臂 × XC/EPS/EAT × fold0/1，最多 30 次 unit 启动 / 900 epochs；明确禁止新增预训练、重跑、P3、OOF、refit 和 outer-test。执行前按项目规则安全同步：`git pull --ff-only origin dev` 返回 `Already up to date`；现场基线为 `dev@2e8a5aa98921a836936d344b7cd8c8eac135e131`，工作区干净。

**只读预检结果**：

- 四个 5,000-step 部署包当前 SHA 与 `Plan.md` 登记值完全匹配：GLT_REF `7dc016dc422c58a6bba705a7a69c9f21051231350393a93d621b33e973883c47`、CAT `eed6276565f0bc827fc1f56056a25cd8469db184b0e334d04c3dd0f3cfea0e49`、GATE `312ea6708ee1b930001c1963714dfbdee44d8de711c5293bc0378be7a73b24e8`、XATTN `9f2bea298b7341eaaf02e85a84f95974334e64b5436310127d30d82ae49f206e`。CPU strict-load 对 GLT_REF、M_CAT、M_GATE、M_XATTN 均通过；O8_ONLY 从 GLT_REF 包 strict-load 后复制 O8 张量 83/83。O8_ONLY 与三个 MCL arm 的共同 512-wide head 初值 SHA256 为 `1541ef7e59e5a5857275d9e473ecf62d4ca39ea2459ba7cb8baff60143be6437`。
- 统计文件 SHA256 为 `9dc8160f1de5152f6c04a963c40569bac8facd21e68a700f40186363798cf8b1`。XC、EPS、EAT split manifest 均标记 `outer5_inner20`，fold0/1 的 manifest 内部 resolver 互斥/覆盖检查通过；该检查传入 manifest 自身的 `sample_count`，未独立核对 live cohort 行数。
- `results/mcl_ph_20260921/p2/development_r10r3/` 与专用日志根不存在；`Uni-Poly` session 可用。未发现 MCL-PH 训练进程；四卡 GPU 利用率 0%，其中 GPU 3 显示 506 MiB 使用但无对应 compute process。没有启动 unit、创建训练目录或写训练产物。计划要求实际检查的窗口顺序因此尚未开始。
...
### 13.22 DEV2 行偏移索引与 8×5 微调能力（Codex；2026-09-23 UTC）

本节续接 §13.21，保留旧阻断发生时的真实记录。用户本轮要求 Codex 直接解决问题并提供完整 8 任务×5 折微调能力。修改前 `dev@98c0e08` 与 `origin/dev` 一致，`git pull --ff-only origin dev` 成功，工作区干净。

- 新建冻结 cohort 的 JSONL 行偏移索引，准备阶段一次性扫描完整源文件**原始字节**、核对 SHA256/行数并记下每行 offset/length；不解码标签。真实索引位于 `results/mcl_ph_20260921/p2/cohort_record_index.json`，原文件 SHA 为 `d9ed6fa43c1b038f8047f8a89bcdb9e8f1435250c2eb26633df2530d94d0daa2`，6,265 行。准备 window `Uni-Poly:mcl_ph_record_index`，命令见 `scripts/build_mcl_ph_record_index.py`，日志 `logs/mcl_ph_20260921/cohort_record_index.log`，真实 exit 0。旧 cohort 与训练包未修改。
- `development`/`full8x5` 微调运行强制提供该索引；运行时检查 frozen manifest、source stat、keys 与 selected-row 身份，只对当前 fold 的 train/validation 字节区间作无缓冲 seek/read，不再整文件扫描 JSONL 或计算其 SHA。source 身份改变时 fail closed；该路径的信任基础是构建时全文件 SHA 与后续 source device/inode/size/mtime 不变。准备阶段已扫描的 outer-test **字节**仍明确计入事实；“训练读取路径不请求这些字节”不等于“整个项目从未读取这些字节”。
- 微调器新增 `full8x5`：8 task × 5 folds，每臂 40 units、最多 1,200 epochs；原 `development` 仍为 3 task × 2 folds。新增串行 launcher 要求显式臂、包与全新输出/日志根，逐 unit 留真实退出码并验收；全量 aggregator 只验收 40/臂并报告 inner-validation R²/Macro8，`outer_test=NOT_RUN`；不运行外层测试、不生成 OOF、不重选 P2 parent。正式执行 8×5 的臂集合、预算、目录和评价协议仍待独立授权。
- 局部测试 **20 passed**；真实 `egc/fold4` 选择性 loader + 静态 cache 绑定通过（2,163 train、541 validation），全部 8×5 split 与 live task counts 匹配。没有训练、forward/backward、smoke 或 aggregate 真实单元；development 仍 **0/30 starts、0/900 epochs**。旧 `tests/test_mcl_ph_protocol.py` 的两项预训练 launcher fixture 在本轮扩大测试时以 exit 6 失败，原期望分别为 7/0；该轮中断后记录为 2 failed、8 passed。另一次只选该文件的聚合/split 检查在约 60 秒后中断，为 6 passed、10 deselected。两次均不称全量通过。

### 13.23 FULL8X5-1 五臂内部验证微调启动（Codex；2026-09-23 UTC）
