...
轴长、投影退化有独立错误原因。
坐标审计只检查参照投影方向与声明 Stereo 的符号一致性，不设置角度质量阈值：
`expected_sign * cosine > 0` 通过，负值报告方向冲突，恰好为零报告正交投影无法判定。
非有限值、轴长和投影退化继续报错；通过该检查不代表构象质量已经验证。

审计报告区分 `ANOMALY`（已发现异常）与 `REVIEW`（策略或有限链差异待复核），
提前返回的检查显式为 `NOT_RUN`。真实模型 smoke 只需至少一条有效中心键样本，
不强制具有 E/Z 或 N=0；若提供 N=0，仍须同时满足原始结构内部键数为零、模型读出
键数为零。修复 LMDB 初始化失败清理、
...
### 2026-09-12：精简正式运行前置条件（实现，未运行）

`validate_dual_glt.py`、`validate_glt_dual_pretrain.py` 现在接受 1–2 条真实冻结记录。
模型 smoke 至少需要一条有效中心内部键样本，不再强制寻找“中心 E/Z + 真实 N=0”
配对。实际 E/Z/N=0 覆盖情况仍写入报告；已有合成边界回归保留。
前一节的双记录命令仍可使用，但该配对不再是强制前置。

默认审计不再构建旧 `build_star_linking_mol`，对应状态为 `NOT_APPLICABLE`。
继续检查实际开放 Trimer 的物理连接、规范化身份、完整键集合和有效几何。
已声明 Stereo 的样本仍接受上述投影符号审计；已知失败记录须在当前规则下重新审计，
不能自动宣称通过、删除或变更 geometry_valid。不能通过换 fixture 宣称原异常已解决。

2026-09-12 后续修改：已移除 Stereo 坐标审计的角度质量阈值逻辑，没有新增质量告警
阈值或配置开关。新增 E/Z 合成用例，覆盖弱同向投影通过、反向冲突、正交无法判定和
冻结坐标不被修改；本次测试及真实记录复审均未执行，历史通过记录不代表本次验证通过。

正式预训练的最小前置：

1. 执行环境可用，PyTorch/CUDA/PyG/scatter/RDKit/LMDB 版本相容；真实样本清单和
   冻结 topology/Trimer 路径明确，身份契约匹配；已知数据异常须有明确处置。
2. 本次修改相关测试通过即可，不重复要求全部历史测试。保留身份/缺边、mask 泄漏、
   loss 归约、加载和 split 隔离等与当前风险相关的回归。
3. 1–2 条真实记录完成输入与模型调用验证。一次使用实际精度和并行配置的 2–3 update
   smoke，可同时验证两种融合的前后向、非有限检查、优化器、调度器、保存与加载；
   不必重复 CPU/GPU 多套同类验收。仅多卡生产需要真实 DDP 验证。
4. 若计划依赖断点续训，做一次短恢复比较；使用合理数值容差，不要求 CUDA 位级相等。
   完整归因探针、逐任务梯度竞争诊断、额外迁移测试及新 O8 多路径方案不是开跑门槛。
5. 固定 concat/kfuse 各自配置、有效 batch、步数、seed 和输出路径；在有日志的
   tmux `Uni-Poly` 独立 window 中运行，不覆盖已有产物。

正式微调的额外前置：对应模式的真实预训练部署包（默认 5000-step）、八任务标签与
冻结数据可读、outer5_inner20 manifest 和样本顺序匹配。每种融合做 eat/fold0、最多
两 epoch 的 train/validation smoke，可同时验证新性质头、train-only scaler、参数组、
validation 选模及权重恢复，不访问 outer-test。通过后按正式授权执行各 40 folds；
不能要求完整 OOF 结果作为启动五折的前提，OOF 完整性属于运行后的验收。
