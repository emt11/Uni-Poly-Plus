...
class GINConv(MessagePassing):
    ...
    def __init__(self, emb_dim, out_dim, aggr="add", **kwargs):
        ...
        self.edge_encoder = torch.nn.Linear(NUM_BOND_FEATURES, emb_dim)
        torch.nn.init.xavier_uniform_(self.edge_encoder.weight.data)
        torch.nn.init.zeros_(self.edge_encoder.bias.data)

    def forward(self, x, edge_index, edge_attr):
        if edge_attr.size(1) != NUM_BOND_FEATURES:
            raise ValueError(
                f"Expected edge_attr with {NUM_BOND_FEATURES} features, "
                f"got {edge_attr.size(1)}. Reprocess graph data with the current feature extractor."
            )
        ...
        self_loop_attr = torch.zeros(x.size(0), NUM_BOND_FEATURES, device=edge_attr.device, dtype=edge_attr.dtype)
...
class GNN(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0, gnn_type="gin"):
        ...
        self.x_encoder = torch.nn.Linear(NUM_ATOM_FEATURES, emb_dim)
        torch.nn.init.xavier_uniform_(self.x_encoder.weight.data)
        torch.nn.init.zeros_(self.x_encoder.bias.data)
    ...
    def forward(self, *argv):
        ...
        if x.size(1) != NUM_ATOM_FEATURES:
            raise ValueError(
                f"Expected x with {NUM_ATOM_FEATURES} atom features, "
                f"got {x.size(1)}. Reprocess graph data with the current feature extractor."
            )

        x = self.x_encoder(x.float())
        h_list = [x]
