"""Standalone MTS-GLT-v1 pretraining loop.

This loop intentionally stays separate from B0-v2: the two routes share data
and checkpoint utilities, not a unified Trainer abstraction.
"""
...
class MTSGLTPretrainContainer(nn.Module):
    def __init__(self, model, atom_head, line_head, o8_projection, glt_projection):
        super().__init__()
        self.model = model
    ...
    def forward(self, data, args, micro_step, generator=None):
        ...
        line_inputs = make_masked_line_inputs(
            data, float(args.glt_line_mask_ratio), generator=generator
        )
        z_o8, z_glt, atom_states, line_states = self.model.forward_pretrain(
            data,
            atom_mask=atom_mask,
        ...
        atom_sum, atom_detached, atom_count, atom_correct = _joint_masked_atom_terms(
            data, atom_states, self.atom_head, atom_mask
        )
        line_sum, line_count, line_correct = masked_line_loss(
            line_states, data.glt_token_label, line_inputs["selected"], self.line_head
        )
        contrastive, pool_size = distributed_bidirectional_infonce(
            self.o8_projection(z_o8),
            self.glt_projection(z_glt),
...
def _checkpoint_payload(container, optimizer, scheduler, step, args, world_size):
    return {
        "schema": "mts-glt-v1-train-state-v1",
        "model_state": {
...
def run_glt_pretrain(args):
    from src.dataset import UniDataset
    from src.modules import GLTMaskedLineHead, MTSGraphLineModel
    ...
    if str(args.config_schema) != "mts-glt-v1":
        raise ValueError("MTS-GLT-v1 requires its explicit config schema")
    ...
    if distributed:
        ...
        if world_size != 3:
            raise ValueError("MTS-GLT-v1 formal execution requires three ranks")
    else:
        local_rank, rank, world_size = 0, 0, 1
    try:
        set_global_seed(int(args.seed))
        if not torch.cuda.is_available():
            raise RuntimeError("MTS-GLT-v1 training requires CUDA")
        device = torch.device("cuda", local_rank)
        dataset = UniDataset(**dataset_kwargs_from_args(args))
        if len(dataset) != 995799:
            raise RuntimeError(f"MTS-GLT-v1 requires full PI1M_v2, got {len(dataset)}")
        if getattr(dataset, "_periodic_line_glt_sidecar", None) is None:
            raise RuntimeError("MTS-GLT-v1 requires the periodic line sidecar")
        effective_batch = (
        ...
        loader = get_data_loader(
            ...
            batch_size=int(args.batch_size), shuffle=sampler is None,
            drop_last=True, num_workers=int(args.loader_workers),
        ...
        # The downstream-only fusion and MD200 residual are excluded from
        # pretraining, while both complete O8 and GLT branches are trainable.
        for module in (
            model.glt_fusion_norm, model.glt_fusion_projection, model.o8.md_residual
        ):
            for parameter in module.parameters():
                parameter.requires_grad = False
        model.glt_gate.requires_grad = False
        atom_head = nn.Linear(512, int(model.o8.masked_atom_classes))
        line_head = GLTMaskedLineHead(512)
        o8_projection = nn.Sequential(
            nn.LayerNorm(512), nn.Linear(512, int(args.glt_projection_dim))
        )
        glt_projection = nn.Sequential(
            nn.LayerNorm(512), nn.Linear(512, int(args.glt_projection_dim))
        )
        container = MTSGLTPretrainContainer(
            model, atom_head, line_head, o8_projection, glt_projection
        ).to(device)
        ...
        optimizer = torch.optim.Adam(
            ...
            lr=float(args.lr), betas=(0.9, 0.98), eps=1e-8,
        ...
        if args.resume_state:
            ...
            if payload.get("schema") != "mts-glt-v1-train-state-v1":
                raise RuntimeError("MTS-GLT-v1 resume schema mismatch")
            if int(payload["world_size"]) != world_size:
                raise RuntimeError("MTS-GLT-v1 resume world-size mismatch")
            if int(payload["gradient_accumulation"]) != int(args.gradient_accumulation_steps):
                raise RuntimeError("MTS-GLT-v1 resume accumulation mismatch")
        ...
        if rank == 0 and metrics_path.exists() and not args.resume_state:
            raise RuntimeError(f"refusing to append to existing GLT trajectory: {metrics_path}")
        epoch = 0
        ...
        run_steps = min(total_steps, stop_steps) if stop_steps else total_steps
        while optimizer_step < run_steps:
            ...
            record = {
                "step": optimizer_step,
                "loss": sums["atom"] / max(1, counts["atom"])
                + sums["line"] / max(1, counts["line"])
                + sums["infonce"] / int(args.gradient_accumulation_steps),
            ...
            if rank == 0:
                ...
                print(
                    f"MTS-GLT-v1 step={optimizer_step}/{total_steps} "
                ...
                if optimizer_step % int(args.checkpoint_interval_steps) == 0 or optimizer_step == run_steps:
                    _atomic_torch_save(
                        _checkpoint_payload(
                ...
                if optimizer_step in tuple(args.glt_probe_steps):
                    _atomic_torch_save(
                        {"state_dict": container.state_dict(), "step": optimizer_step},
                        result_root / f"mts_glt_v1_probe_{optimizer_step // 1000:03d}k.pth",
                    )
        if distributed:
            dist.barrier()
    finally:
...
__all__ = ["MTSGLTPretrainContainer", "run_glt_pretrain"]
